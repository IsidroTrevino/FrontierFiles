...
<svelte:head>
	<title>Sign In - FrontierFiles</title>
</svelte:head>

<div class="min-h-screen gradient-bg overflow-hidden">
	<!-- Decorative elements -->
	<div class="absolute inset-0 overflow-hidden pointer-events-none">
		<div class="absolute top-20 right-20 w-72 h-72 bg-red-500/10 rounded-full blur-3xl float-animation"></div>
		<div class="absolute bottom-20 left-20 w-96 h-96 bg-rose-500/10 rounded-full blur-3xl" style="animation: float 8s ease-in-out infinite;"></div>
	</div>
	...
	<div class="relative min-h-screen flex items-center justify-center p-4">
		...
		<div class="glass rounded-2xl p-8 md:p-10 w-full max-w-md shadow-2xl fade-in">
			...
			<div class="text-center mb-8">
				...
				<h2 class="text-3xl font-bold text-slate-100 mb-2">Welcome back</h2>
			...
			<form on:submit|preventDefault={handleLogin} class="space-y-5">
				<div class="space-y-2">
					...
					<input
						...
						required
						class="w-full px-4 py-3 bg-slate-900/50 border border-slate-700 rounded-xl text-slate-100 placeholder-slate-500 focus:border-red-500 focus:ring-2 focus:ring-red-500/20 transition-all duration-200"
					/>
				</div>
				...
				<button
					type="submit"
					disabled={loading}
					class="modern-btn w-full py-3.5 px-6 rounded-xl font-semibold bg-gradient-to-r from-red-500 to-rose-500 text-white hover:shadow-lg hover:shadow-red-500/50 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 flex items-center justify-center gap-2"
				>
					{#if loading}
			...
			<div class="relative my-8">
				...
				<div class="relative flex justify-center text-sm">
					<span class="px-4 bg-slate-900/50 text-slate-400">Don't have an account?</span>
				</div>
			</div>
